...
const currentBandContainer =
    document.getElementById("current-band-container");
...
async function loadCurrentBand() {
    ...
    if (!trade) {
        ...
        currentBandContainer.innerHTML = `
            <p>
                Select a trade to view its current price band.
            </p>
        `;
    ...
    try {
        ...
        if (response.status === 404) {
            ...
            currentBandContainer.innerHTML = `
                <div class="no-band">
                    <p>
                        No price band has been set for
                        <strong>${formatTrade(trade)}</strong>.
                    </p>
                </div>
            `;
        ...
        if (!response.ok) {
            ...
            throw new Error(
                data.detail || "Could not load price band."
            );
        ...
        currentBandContainer.innerHTML = `
            <div class="band-card">
                ...
                <h3>
                    ${formatTrade(data.trade)}
                </h3>

                <p>
                    <strong>Minimum:</strong>
                    ₹${Number(data.floor).toFixed(2)}
                </p>

                <p>
                    <strong>Maximum:</strong>
                    ₹${Number(data.ceiling).toFixed(2)}
                </p>
        ...
        currentBandContainer.innerHTML = `
            <p class="error-text">
                Could not load current price band.
            </p>
        `;
...
async function savePriceBand() {
    ...
    try {
        ...
        if (!response.ok) {
            ...
            throw new Error(
                data.detail || "Could not save price band."
            );
