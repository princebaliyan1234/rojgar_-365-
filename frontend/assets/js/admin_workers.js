...
async function loadWorkers() {
  ...
  try {
    const response = await fetch(`${API_BASE_URL}/unions/${unionId}/workers`);
    ...
    document.getElementById("union-info").textContent = `${data.length} worker(s) found`;

    // =========================
    // SEPARATE BY KYC STATUS
    // =========================
    const pendingWorkers = data.filter(worker => worker.kyc_status === "pending");
    const approvedWorkers = data.filter(worker => worker.kyc_status === "approved");
    const rejectedWorkers = data.filter(worker => worker.kyc_status === "rejected");
    ...
    pendingSection.innerHTML = `
      <h2>Pending KYC</h2>
      <p class="section-description">Workers waiting for verification.</p>
    `;
    ...
    if (pendingWorkers.length === 0) {
      ...
      pendingWorkers.forEach(worker => {
        pendingSection.innerHTML += createWorkerCard(worker, true);
      });
    }
    ...
    approvedSection.innerHTML = `
      ...
      <p class="section-description">Workers whose KYC has been approved.</p>
    `;
    ...
    if (approvedWorkers.length === 0) {
      ...
      approvedWorkers.forEach(worker => {
        approvedSection.innerHTML += createWorkerCard(worker, false);
      });
    }
    ...
    rejectedSection.innerHTML = `
      ...
      <p class="section-description">Workers whose KYC has been rejected.</p>
    `;
    ...
    if (rejectedWorkers.length === 0) {
      ...
      rejectedWorkers.forEach(worker => {
        rejectedSection.innerHTML += createRejectedWorkerCard(worker);
      });
    }
    ...
    container.innerHTML = `
      <div class="error-state">
        ...
        <button onclick="loadWorkers()">Try Again</button>
      </div>
    `;
  }
}
  ...
  return `
    <div class="worker-card">
      <div class="worker-info">
        ...
        <p><strong>Trade:</strong> ${escapeHtml(worker.trade)}</p>
        <p>
      ...
      ${showActions ? `
        <div class="worker-actions">
          <button class="approve-btn" onclick="updateKyc(${worker.id}, 'approved')">Approve</button>
          <button class="reject-btn" onclick="updateKyc(${worker.id}, 'rejected')">Reject</button>
        </div>
      ` : ""}
    </div>
  `;
  ...
  return `
    <div class="worker-card">
      <div class="worker-info">
        ...
        <p>
          <strong>KYC Status:</strong>
          <span class="kyc-status rejected">✗ Rejected</span>
        </p>
      </div>
...
async function updateKyc(workerId, status) {
  const confirmation = confirm(`Are you sure you want to ${status} this worker's KYC?`);
  ...
  try {
    const response = await fetch(`${API_BASE_URL}/workers/${workerId}/kyc`, {
      method: "PATCH",
      headers: {
    ...
    if (!response.ok) {
      throw new Error(data.detail || "Could not update KYC.");
    }
